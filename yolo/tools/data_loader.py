...
class YoloDataset(Dataset):
    ...
    def filter_data(self, dataset_path: Path, phase_name: str, sort_image: bool = False) -> list:
        ...
        logger.info(f"Recorded {valid_inputs}/{n_images} valid inputs")
        return data
...
# class YoloDatasetOnline(Dataset):
#     def __init__(self, data_cfg: DataConfig, dataset_cfg: DatasetConfig, phase: str = "train2017"):
#         augment_cfg = data_cfg.data_augment
#         self.image_size = data_cfg.image_size
#         phase_name = dataset_cfg.get(phase, phase)
...
#         repeat = 1
#         seed = 1
#         with open(f"./collections/{dataset_cfg['train']}_sigma{sigma}_repeat{repeat}_seed{seed}.json", "r") as f:
#             import json
...
#         count = 0
#         for new_data in self.stream_data['stream']:
#             count += 1
#             image_id = new_data['file_name'].split("/")[1]
#             image_info = image_info_dict.get(image_id, None)
#             if image_info is None:
#                 continue
...
#             if data_type != "json":
#                 raise ValueError(f"Unsupported data_type: {data_type}. Only 'json' is supported.")
...
def create_dataloader(data_cfg: DataConfig, dataset_cfg: DatasetConfig, task: str = "train", online=False):
    if task == "inference":
