...
class YOLOLoss:
    ...
    def __call__(self, predicts: Union[List[Tuple[Tensor, Tensor, Tensor]], Tuple[Tensor, Tensor, Tensor]], targets: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        # Unwrap if it's a list with one tuple
        if isinstance(predicts, list) and isinstance(predicts[0], (tuple, list)):
            predicts = predicts[0]  # unwrap the single prediction

        predicts_cls, predicts_anc, predicts_box = predicts

        predicts_cls_detached = predicts_cls.detach()
        predicts_box_detached = predicts_box.detach()

        align_targets, valid_masks = self.matcher(targets, (predicts_cls_detached, predicts_box_detached))
...
class DualLoss:
    def __init__(self, cfg: Config, vec2box, num_classes=None) -> None:
        loss_cfg = cfg.task.loss
        self.loss = YOLOLoss(loss_cfg, vec2box, class_num=num_classes or cfg.dataset.class_num, reg_max=cfg.model.anchor.reg_max)
...
def create_loss_function(cfg: Config, vec2box, num_classes=None) -> DualLoss:
    # TODO: make it flexible, if cfg doesn't contain aux, only use SingleLoss
    loss_function = DualLoss(cfg, vec2box, num_classes)
