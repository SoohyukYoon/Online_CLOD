...
class BoxMatcher:
    ...
    def get_iou_matrix(self, predict_bbox, target_bbox) -> Tensor:
        ...
        """
        # print("[DEBUG] predict_bbox shape:", predict_bbox.shape)
        # print("[DEBUG] target_bbox shape:", target_bbox.shape)
    ...
    def __call__(self, target: Tensor, predict: Tuple[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        # print(f"[DEBUG] target_matrix shape: {target_matrix.shape}")
        # print(f"[DEBUG] grid_mask shape: {grid_mask.shape}")
        # print(f"[DEBUG] topk_mask sum (per anchor): {topk_mask.sum(dim=1)}")
        # print(f"[DEBUG] unique_indices: {unique_indices}")
        # print(f"[DEBUG] valid_mask (sum): {valid_mask.sum(dim=1)}")
...
class Vec2Box:
    ...
    def __call__(self, predicts):
        ...
        # print(f"[DEBUG] preds_cls shape: {preds_cls.shape}")
        # print(f"[DEBUG] preds_box shape before transform: {pred_LTRB.shape}")
        # print(f"[DEBUG] anchor_grid shape: {self.anchor_grid.shape}")
        # print(f"[DEBUG] preds_box shape after transform: {preds_box.shape}")
...
class Anc2Box:
    ...
    def __call__(self, predicts: List[Tensor]):
        ...
        for layer_idx, predict in enumerate(predicts):
            ...
            # print(f"[DEBUG] Layer {layer_idx}: pred_box shape: {pred_box.shape}")
            # print(f"[DEBUG] Layer {layer_idx}: pred_cls shape: {pred_cls.shape}")
            # print(f"[DEBUG] Layer {layer_idx}: anchor_grid shape: {self.anchor_grids[layer_idx].shape}")
