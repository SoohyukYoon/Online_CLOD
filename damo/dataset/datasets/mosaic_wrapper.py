...
class PseudoMosaicWrapper(MosaicWrapper):
    def __getitem__(self, inp):
        if type(inp) is tuple:
            enable_mosaic_mixup = inp[0]
            idx = inp[1]
        ...
        if enable_mosaic_mixup:
            ...
            # -----------------------------------------------------------------
            if (not len(labels) == 0 and random.random() < self.mixup_prob):
                img, labels = self.mixup(img, labels, self.input_dim, model=model, score_thresh=score_thresh)
    ...
    def mixup(self, origin_img, origin_labels, input_dim, model=None, score_thresh=0.7):
        jit_factor = random.uniform(*self.mixup_scale)
        FLIP = random.uniform(0, 1) > 0.5
        cp_labels = []
        while len(cp_labels) == 0:
            cp_index = random.randint(0, self.__len__() - 1)
            cp_labels = self._dataset.load_anno(cp_index, model=model, score_thresh=score_thresh)
        img, cp_labels, _, _ = self._dataset.pull_item(cp_index)
