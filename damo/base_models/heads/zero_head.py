...
class ZeroHeadHarmonious(nn.Module):
    """Ref to Generalized Focal Loss V2: Learning Reliable Localization Quality
    Estimation for Dense Object Detection.
    """
    def __init__(
        ...
        super(ZeroHeadHarmonious, self).__init__()
        self.integral = Integral(self.reg_max)
        self.loss_dfl = DistributionFocalLoss(loss_weight=0.25)
        self.loss_cls = QualityFocalLoss(use_sigmoid=False,
                                         beta=2.0,
    ...
    def forward(self, xin, labels=None, imgs=None, aux_targets=None, score_weights=None):

        
        if self.training:
            return self.forward_train(xin=xin, labels=labels, imgs=imgs, score_weights=score_weights)
        else:
            return self.forward_eval(xin=xin, labels=labels, imgs=imgs)

    def forward_train(self, xin, labels=None, imgs=None, aux_targets=None, score_weights=None):
    ...
    def loss(
        ...
        bbox_weights = bbox_weights.max(dim=1)[0] # torch.Size([134400])
        

        cls_scores = cls_scores.reshape(-1, self.cls_out_channels) # torch.Size([134400, class num])
        ...
        bbox_weight_pseudo = bbox_weights[pos_inds]
        weight_targets *= bbox_weight_pseudo
    ...
    def get_targets(self,
        ...
        #     print(gt_bboxes_list[i].shape)
        #     print(gt_labels_list[i].shape)
        # breakpoint()
    ...
    def get_target_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            bbox_w = torch.zeros_like(pos_bbox_targets)
            
            for i in range(len(pos_assign_gt_inds)):
                bbox_w[i] = score_weights[pos_assign_gt_inds[i]]
