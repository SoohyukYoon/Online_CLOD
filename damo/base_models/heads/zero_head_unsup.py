...
# from methods.baseline2_freq_balanced_pseudo_tia import _ImageLayer1, _ImageLayer2, _ImageLayer3, ReverseLayerF
...
class ZeroPseudoHead(nn.Module):
    ...
    def __init__(
        ...
        super(ZeroPseudoHead, self).__init__()
    ...
    def forward_train(self, xin, labels=None, imgs=None, aux_targets=None):
        ...
        img_feat1, _ = self.DA_img1(ReverseLayerF.apply(base_feat1, 1.0))
        img_feat2, _ = self.DA_img2(ReverseLayerF.apply(base_feat2, 1.0))
        img_feat3, _ = self.DA_img3(ReverseLayerF.apply(base_feat3, 1.0))
        ...
        return loss, (base_feat1, base_feat2, base_feat3), (img_feat1, img_feat2, img_feat3)
...
class _ImageLayer1(nn.Module):
    def __init__(self, num_cls=2):
        ...
        self.conv1 = nn.Conv2d(128, 128, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv2 = nn.Conv2d(128, 128, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv3 = nn.Conv2d(128, num_cls, kernel_size=1, stride=1, padding=0, bias=False)
        self._init_weights()
...
class _InstanceLayer(nn.Module):
    def __init__(self, dim=1024, num_cls=2):
        super(_InstanceLayer, self).__init__()
        self.conv1 = nn.Conv2d(dim, 256, kernel_size=3, stride=2, padding=1, bias=False)
        self.conv2 = nn.Conv2d(256, 128, kernel_size=3, stride=2, padding=1, bias=False)
