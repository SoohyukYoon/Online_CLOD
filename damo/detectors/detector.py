...
from damo.base_models.heads import build_head, build_head_harmonious
...
class HarmoniousDetector(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.backbone = build_backbone(config.model.backbone)
        self.neck = build_neck(config.model.neck)
        self.head = build_head_harmonious(config.model.head)
    ...
    def forward(self, x, targets=None, tea=False, stu=False, score_weights=None):
        images = to_image_list(x)
        feature_outs = self.backbone(images.tensors)  # list of tensor
        fpn_outs = self.neck(feature_outs)
...
def build_local_model_harmonious(config, device):
    model = HarmoniousDetector(config)
    model.init_model()
    model.to(device)
