...
class BASELINEFreqBalancedPseudoGRAM2(BASELINE2):
    def initialize_memory_buffer(self, memory_size):
        ...
        self.gram_extractor = GramMatrixExtractor()
    ...
    def online_train(self, sample, batch_size, n_worker, iterations=1, stream_batch_size=1):
        ...
        src_batch_size = min(len(self.memory.buffer_source), int(batch_size/4)) # half or full
        tgt_batch_size = min(len(self.memory.buffer), int(batch_size/4 - stream_batch_size))
        for i in range(iterations):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                # Compute Gram matrix contrastive loss
                gram_loss = compute_gram_loss(
                    (base_feat1_src[:src_batch_size], base_feat2_src[:src_batch_size], base_feat3_src[:src_batch_size]),
                    (base_feat1_tgt, base_feat2_tgt, base_feat3_tgt),
                    supervised_targets[:src_batch_size],
                    unsup_targets,
                    self.gram_extractor,
                    self.supcon_loss
                )
...
class GramMatrixExtractor:
    ...
    def __init__(self, input_size=640, device='cuda'):
        ...
        # self.DA_filter1 = Filter_conv1(8256).to(device)
        # for p in self.DA_filter1.parameters():
        #     p.requires_grad = False
        # self.DA_filter2 = Filter_conv1(32896).to(device)
        # for p in self.DA_filter2.parameters():
        #     p.requires_grad = False
        # self.DA_filter3 = Filter_conv1(131328).to(device)
        # for p in self.DA_filter3.parameters():
        #     p.requires_grad = False
        # self.DA_filters = {
        #     1: self.DA_filter1,
        #     2: self.DA_filter2,
    ...
    def compute_gram_matrix(self, features):
        ...
        triu_indices = torch.triu_indices(c, c, device=gram.device)
        # Extract upper triangular values for each batch
        gram_upper = gram[:, triu_indices[0], triu_indices[1]]
        return gram_upper
        # return gram
    ...
    def extract_bbox_features(self, feature_map, targets, feat_level):
        ...
        for batch_idx in range(batch_size):
            if batch_idx < len(targets):
                bboxes = targets[batch_idx].bbox  # Assuming shape (N, 4) with [x1, y1, x2, y2]
                labels = targets[batch_idx].extra_fields['labels']
...
# Integration into your training function
def compute_gram_loss(base_feats_src, base_feats_tgt,
                      supervised_targets, unsup_targets, gram_extractor, supcon_loss):
    """
    Compute Gram matrix contrastive loss.
    """
    total_gram_loss = 0.0
    
    # Process source supervised features
    if base_feats_src[0] is not None:
        gram_src, labels_src = gram_extractor.compute_gram_matrices_for_targets(
            base_feats_src, supervised_targets
        )
        
        # Process target unsupervised features
        gram_tgt, labels_tgt = gram_extractor.compute_gram_matrices_for_targets(
            base_feats_tgt, unsup_targets
        )
        
        # Combine gram matrices and labels from different batches
        for feat_level in gram_src.keys():
            if feat_level in gram_tgt:
                combined_grams = torch.cat([gram_src[feat_level], gram_tgt[feat_level]], dim=0)
                # combined_grams =
                # Create labels: 0 for source, 1 for target
                # combined_labels = [0] * len(labels_src) + [1] * len(labels_tgt)
                combined_labels = labels_src + labels_tgt
                
                # Compute contrastive loss
                if len(combined_grams) > 1:
                    loss = supcon_loss(combined_grams, combined_labels)
                    total_gram_loss += loss
    
    # Average over feature levels
    if total_gram_loss > 0:
        total_gram_loss = total_gram_loss / 3.0
    
    return total_gram_loss
