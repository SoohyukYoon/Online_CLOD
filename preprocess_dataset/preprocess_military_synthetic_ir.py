...
def filter_dataset(original_path, output_path, target_dataset):
    ...
    for i in range(len(labels_data['images'])):
        if labels_data['images'][i]['sensor'] == 2 and \
            labels_data['images'][i]['attributes']['season'] == 1 and \
            labels_data['images'][i]['attributes']['night'] == 2:
...
if __name__ == "__main__":
    ...
    original_path = Path("./data/military_synthetic")
    output_path   = Path("./data/military_synthetic_domain_infrared")
